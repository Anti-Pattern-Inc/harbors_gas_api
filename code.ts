...
function reservedEvent (eventName: string, name: string, startDate: Date, endDate: Date, meet: boolean) {
  ...
  try {
    ...
    // カレンダー登録
    const noticeName: string = eventName + '-' + name + '様'
    const event = createEventToCalendar(noticeName, startDate, endDate, meet)
    if (meet) {
      // MeetのUrlを取得
...
function sendAttachEmail (mailAddress: string,
  ...
  mailTemplateId: string): void{
  // 添付ファイルを取得
  const arrReport:Array<GoogleAppsScript.Drive.File> = []
  try {
